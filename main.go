...
func TestClient(t *testing.T) {
	...
	if cnsClient.serviceClient.Version != cns.ReleaseVSAN67u3 {
		...
		reCreateTaskInfo, err := cns.GetTaskInfo(ctx, recreateTask)
		if err != nil {
			t.Errorf("Failed to create volume. Error: %+v \n", err)
			t.Fatal(err)
		}
		reCreateTaskResult, err := cns.GetTaskResult(ctx, reCreateTaskInfo)
		if err != nil {
			t.Errorf("Failed to create volume. Error: %+v \n", err)
			t.Fatal(err)
		}
	...
	if cnsClient.serviceClient.Version != cns.ReleaseVSAN67u3 && cnsClient.serviceClient.Version != cns.ReleaseVSAN70 {
		...
		queryVolumeInfoTaskResults, err := cns.GetTaskResultArray(ctx, queryVolumeInfoTaskInfo)
		if err != nil {
			t.Errorf("Failed to query volumes with QueryVolumeInfo. Error: %+v \n", err)
			t.Fatal(err)
		}
